...
# RISPETTO AL 5 HO CAMBIATO ALCUNE COSE IN CHECK FUZZY MATCHING
...
def check_fuzzy_match(stringa1, stringa2):
    ...
    for parola in parole_da_rimuovere:
        if parola in stringa1_no_parentesis:
            stringa1_no_parentesis.replace(parola, "")
        if parola in stringa2_no_parentesis:
            stringa2_no_parentesis.replace(parola, "")
    ...
    score = fuzz.ratio(stringa1_no_parentesis, stringa2_no_parentesis)
    ...
    if (
        ...
        or words1[0] in words2[0]
        or words2[0] in words1[0]
    ):
        ...
        ingredient_Planeat = "a"
        ingredient_Dissapore = "a"

        if (
            score != 100
            and len(ingredient_Planeat[0][:-1]) > 3
            and len(ingredient_Dissapore[0][:-1]) > 3
        ):

            # if score != 100 and len(words1[0][:-1]) > 3 and len(words2[0][:-1]) > 3:

            if (
                len(comuni) >= (len(sottostringhe1) + len(sottostringhe2)) / 2
                and len(sottostringhe1) > 2
                and len(sottostringhe2) > 2
            ):
