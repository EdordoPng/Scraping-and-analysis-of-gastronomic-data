...
# Questo programma mi fornisce già un fuzzy più attento e complesso. Qui invece di dare un ontributo
# moltiplicativo come nel fuzzy 3, andiamo a dare una parte fissa
...
def check_fuzzy_match(stringa1, stringa2):
    ...
    if words1[0][:-1] == words2[0][:-1]:
        # Calculate the similarity score using the "partial_ratio" method of the fuzzywuzzy library
        # on the remaining parts of the two strings after the comma
        score = fuzz.partial_ratio(words1[0], words2[0])
        ...
        score = fuzz.ratio(stringa1.lower(), stringa2.lower())
        score = score - 35
        return round(score)
...
def main():
    ...
    complete_similarity_list, ingr_match_counter = list_fuzzy_similarity(
        ingredienti_planeat, ingredienti_dissapore, 50
    )

    print(
        f"Sono state trovate tot corrispondenze : {ingr_match_counter} con una soglia pari a : 52 "
    )
